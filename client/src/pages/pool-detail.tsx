  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Lido-Specific Information */}
        {pool.project === 'lido' && pool.rawData && (
          <div className="space-y-6 mb-8">
            {/* Staking Infrastructure */}
            {pool.rawData.staking && (
              <Card>
                <CardHeader>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="text-center">
                      <p className="text-2xl font-bold text-purple-600">{pool.rawData.staking.validatorCount?.toLocaleString() || 'N/A'}</p>
                      <p className="text-sm text-gray-600">Active Validators</p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-blue-600">{pool.rawData.staking.operatorCount || 'N/A'}</p>
                      <p className="text-sm text-gray-600">Node Operators</p>
                    </div>
                    <div className="text-center">
                      <p className="text-xl font-bold text-green-600">No Major Events</p>
                      <p className="text-sm text-gray-600">Slashing History</p>
                    </div>
                    <div className="text-center">
                      <p className="text-xl font-bold text-orange-600">Self-Insured</p>
            ...
            {/* Withdrawal Queue Information */}
            {pool.rawData.withdrawalQueue && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="text-center">
                      <p className="text-3xl font-bold text-orange-600">
                        {pool.rawData.withdrawalQueue.finalizationInDays?.toFixed(1) || 'N/A'}
                      </p>
                      <p className="text-sm text-gray-600">Days to Finalization</p>
                    </div>
                    <div className="text-center">
                      <p className="text-lg font-semibold text-blue-600">
                        {pool.rawData.withdrawalQueue.status || 'N/A'}
                      </p>
                      <p className="text-sm text-gray-600">Queue Status</p>
                    </div>
                    <div className="text-center">
                      <p className="text-lg font-semibold text-gray-700">
                        {pool.rawData.withdrawalQueue.finalizationAt ? 
                          new Date(pool.rawData.withdrawalQueue.finalizationAt).toLocaleDateString() : 'N/A'}
                      </p>
            ...
            {pool.rawData.historicalAprs && pool.rawData.historicalAprs.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center">
                    <TrendingUp className="w-5 h-5 mr-2 text-green-600" />
                    7-Day APR Trend (Lido)
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="h-40">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={pool.rawData.historicalAprs.map(item => ({
                        date: new Date(item.timeUnix * 1000).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
                        apr: item.apr
                      }))}>
                        ...
                        />
                        <Tooltip 
                          formatter={(value: any) => [`${Number(value).toFixed(3)}%`, 'APR']}
                          contentStyle={{
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-4 pt-4 border-t">
                    <div className="text-center">
                      <p className="text-sm text-gray-500">Latest APR</p>
                      <p className="text-lg font-semibold text-green-600">
                        {pool.rawData.lastApr?.toFixed(3) || 'N/A'}%
                      </p>
                    </div>
                    <div className="text-center">
                      <p className="text-sm text-gray-500">7-Day Average</p>
                      <p className="text-lg font-semibold text-blue-600">
                        {pool.rawData.smaApr?.toFixed(3) || 'N/A'}%
                      </p>
                    </div>
                  </div>
                </CardContent>
            ...
            {pool.rawData.protocol?.riskFactors && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {pool.rawData.protocol.riskFactors.map((risk, index) => (
                  ...
                  <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-sm text-blue-800">
                      <strong>Contract:</strong> {pool.rawData.protocol.contractAddress || 'N/A'}
                    </p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-2">stETH Supply</h4>
                      <p className="text-2xl font-bold text-blue-600">
                        {pool.rawData.liquidity.stethTotalSupply?.toLocaleString() || 'N/A'} ETH
                      </p>
                    </div>
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-2">Staking Ratio</h4>
                      <p className="text-2xl font-bold text-green-600">
                        {pool.rawData.liquidity.stakingRatio ? (pool.rawData.liquidity.stakingRatio * 100).toFixed(1) : 'N/A'}%
                      </p>
                    </div>
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-2">Redemption</h4>
                      <p className="text-sm text-gray-700">{pool.rawData.liquidity.redemptionMechanism || 'N/A'}</p>
                    </div>
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-2">Peg Stability</h4>
                      <p className="text-sm text-gray-700">{pool.rawData.liquidity.peg || 'N/A'}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
